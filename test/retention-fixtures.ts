...
async function deleteFixtures(client: ChClient, projectId: string) {
  ...
  // cleanup deterministic before the next fixture insert starts.
  await client.command({
    query: `ALTER TABLE openpanel.events DELETE WHERE project_id = '${projectId}' SETTINGS mutations_sync = 2`,
  });
  // Materialized views are NOT touched by DELETE FROM events; mutate them too
  // so reruns stay clean.
  await client.command({
    query: `ALTER TABLE openpanel.cohort_events_mv DELETE WHERE project_id = '${projectId}' SETTINGS mutations_sync = 2`,
  });
  await client.command({
